...
\begin{verbatim}
## [1] "ID"                 "Age.at.study.entry" "Sex"               
## [4] "Diagnosis"
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#here get the batch names subdir}
\NormalTok{subdirs }\OtherTok{\textless{}{-}} \FunctionTok{list.dirs}\NormalTok{(maindir, }\AttributeTok{recursive =} \ConstantTok{FALSE}\NormalTok{)}


\NormalTok{all\_files }\OtherTok{\textless{}{-}} \FunctionTok{list.files}\NormalTok{(subdirs, }\AttributeTok{pattern =} \StringTok{"\_cleaned}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{.fcs$"}\NormalTok{, }\AttributeTok{full.names =} \ConstantTok{TRUE}\NormalTok{)}

\NormalTok{file\_names }\OtherTok{\textless{}{-}} \FunctionTok{basename}\NormalTok{(all\_files)}
\NormalTok{file\_subdirs }\OtherTok{\textless{}{-}} \FunctionTok{basename}\NormalTok{(}\FunctionTok{dirname}\NormalTok{(all\_files))}

\NormalTok{meta}\SpecialCharTok{$}\NormalTok{subdir}\OtherTok{\textless{}{-}} \ConstantTok{NA\_character\_}\NormalTok{;meta}\SpecialCharTok{$}\NormalTok{filename }\OtherTok{\textless{}{-}} \ConstantTok{NA\_character\_}

\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \FunctionTok{seq\_len}\NormalTok{(}\FunctionTok{nrow}\NormalTok{(meta))) \{}
\NormalTok{  id }\OtherTok{\textless{}{-}}\NormalTok{ meta}\SpecialCharTok{$}\NormalTok{ID[i]}
  
  \CommentTok{\# Match files starting with the ID}
\NormalTok{  matched\_idx }\OtherTok{\textless{}{-}} \FunctionTok{grep}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(}\StringTok{"\^{}"}\NormalTok{, id), file\_names)}
  
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{length}\NormalTok{(matched\_idx) }\SpecialCharTok{==} \DecValTok{1}\NormalTok{) \{}
\NormalTok{    meta}\SpecialCharTok{$}\NormalTok{filename[i] }\OtherTok{\textless{}{-}}\NormalTok{ file\_names[matched\_idx]}
\NormalTok{    meta}\SpecialCharTok{$}\NormalTok{subdir[i] }\OtherTok{\textless{}{-}}\NormalTok{ file\_subdirs[matched\_idx]}
\NormalTok{  \} }\ControlFlowTok{else} \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{length}\NormalTok{(matched\_idx) }\SpecialCharTok{\textgreater{}} \DecValTok{1}\NormalTok{) \{}
    \FunctionTok{warning}\NormalTok{(}\FunctionTok{sprintf}\NormalTok{(}\StringTok{"Multiple matches for ID \textquotesingle{}\%s\textquotesingle{}. Taking the first."}\NormalTok{, id))}
\NormalTok{    meta}\SpecialCharTok{$}\NormalTok{filename[i] }\OtherTok{\textless{}{-}}\NormalTok{ file\_names[matched\_idx[}\DecValTok{1}\NormalTok{]]}
\NormalTok{    meta}\SpecialCharTok{$}\NormalTok{subdir[i] }\OtherTok{\textless{}{-}}\NormalTok{ file\_subdirs[matched\_idx[}\DecValTok{1}\NormalTok{]]}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \FunctionTok{warning}\NormalTok{(}\FunctionTok{sprintf}\NormalTok{(}\StringTok{"No file match found for ID \textquotesingle{}\%s\textquotesingle{}"}\NormalTok{, id))}
\NormalTok{  \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#metadata with immunophenotyping files}
\NormalTok{metafiles}\OtherTok{\textless{}{-}}\NormalTok{meta[}\FunctionTok{complete.cases}\NormalTok{(meta}\SpecialCharTok{$}\NormalTok{subdir), ]}
\end{Highlighting}
\end{Shaded}
